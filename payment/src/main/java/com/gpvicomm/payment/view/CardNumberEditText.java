 ...
 */
public class CardNumberEditText extends PaymentEditText {
    ...
    public String getCardNumber() {
        return mIsCardNumberValid
                ? PaymentTextUtils.removeSpacesAndHyphens(getText().toString())
                : null;
    }
    ...
    private void listenForTextChanges() {
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                String spacelessNumber = PaymentTextUtils.removeSpacesAndHyphens(s.toString());
    ...
    private void updateCardBrandFromNumber(String partialNumber) {
        String spacelessCardNumber = PaymentTextUtils.removeSpacesAndHyphens(partialNumber);



        if(partialNumber.length() >= 6 && partialNumber.length() <= 10){
            PaymentService paymentService = Payment.getService(mContext);
            paymentService.cardBin(spacelessCardNumber).enqueue(new Callback<CardBinResponse>() {
                @Override
